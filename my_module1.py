...
# =====================================================================
# Multi-System Configuration & Builder (paper §2.1)
# =====================================================================
SYSTEMS_CONFIG = [
    {'conv_idx': 0,  'pool_layer': 'max', 'ker': 2, 'stri': 2},   # S_0: Conv(3→64)
    {'conv_idx': 2,  'pool_layer': 'max', 'ker': 2, 'stri': 2},   # S_1: Conv(64→128)
    {'conv_idx': 4,  'pool_layer': False,  'ker': 2, 'stri': 2},   # S_2: Conv(128→256)
    {'conv_idx': 5,  'pool_layer': 'max', 'ker': 2, 'stri': 2},   # S_3: Conv(256→256)
    {'conv_idx': 7,  'pool_layer': False,  'ker': 2, 'stri': 2},   # S_4: Conv(256→512)
    {'conv_idx': 8,  'pool_layer': 'max', 'ker': 2, 'stri': 2},   # S_5: Conv(512→512)
    {'conv_idx': 10, 'pool_layer': False,  'ker': 2, 'stri': 2},   # S_6: Conv(512→512)
    {'conv_idx': 11, 'pool_layer': 'max', 'ker': 2, 'stri': 2},   # S_7: Conv(512→512)
]


def build_multi_system(n_classes, train_loader):
    """Build single backbone (model7) + 8 auxiliary classifiers."""
    model = m7(n_classes, train_loader)
    conv_idx_set = set(sys['conv_idx'] for sys in SYSTEMS_CONFIG)
    feat_sizes = []

    x_dummy = next(iter(train_loader))[0][:1].float().to(DEVICE)
    with torch.no_grad():
        x_fwd = x_dummy
        for idx, layer in enumerate(model.layers):
            ...
            if idx in conv_idx_set:
                for sys in SYSTEMS_CONFIG:
                    if sys['conv_idx'] == idx:
                        if sys['pool_layer'] == 'max':
                            x_p = f.max_pool2d(x_fwd, sys['ker'], sys['stri'])
                        elif sys['pool_layer'] == 'avg':
                            x_p = f.avg_pool2d(x_fwd, 2, 2)
                        else:
                            x_p = x_fwd
                        feat_sizes.append(x_p.view(1, -1).shape[1])
                        break

    aux_classifiers = []
    for j, fs in enumerate(feat_sizes):
        w = torch.empty(n_classes, fs, device=DEVICE)
        nn.init.xavier_uniform_(w)
        aux_classifiers.append(w)
        sys = SYSTEMS_CONFIG[j]
        print(f'  S_{j}: conv_idx={sys["conv_idx"]}, '
              f'pool={sys["pool_layer"]}, feat={fs}')
    return model, aux_classifiers


def evaluate_all_systems(model, aux_classifiers, systems_config,
                         train_loader, val_loader):
    """Evaluate all systems' accuracy via a single backbone forward pass."""
    J = len(systems_config)
    conv_idx_set = set(s['conv_idx'] for s in systems_config)
    results = {}
            ...
            for images, labels in loader:
                ...
                for idx, layer in enumerate(model.layers):
                    if layer.name in ['conv', 'fc']:
                        ...
                        x_fwd = layer.weights(x_fwd)
                    if idx in conv_idx_set:
                        for j, sys in enumerate(systems_config):
                            if sys['conv_idx'] == idx:
                                if sys['pool_layer'] == 'max':
                                    xp = f.max_pool2d(x_fwd, sys['ker'],
                                                      sys['stri'])
                                elif sys['pool_layer'] == 'avg':
                                    xp = f.avg_pool2d(x_fwd, 2, 2)
                                else:
                                    xp = x_fwd
                                xf = xp.view(xp.size(0), -1)
                                out = torch.sigmoid(
                                    xf @ aux_classifiers[j].t())
                                _, pred = torch.max(out, 1)
                                correct[j] += (
                                    pred == labels).sum().item()
                                break
        results[name] = [100.0 * c / max(total, 1) for c in correct]
