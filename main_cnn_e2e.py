...
a_slope = 1.0      # adjustable ReLU negative slope: starts at 1 (linear), decreases to 0.01
pool_mode = 'avg'   # pooling mode: starts 'avg', switches to 'max' when a→0.01
t0 = time.time()
...
for j in range(0,200):
      ...
      for i in range(0,len(train_loaderD)):
        ...
        try:
             ...
             # ===== Adjustable ReLU schedule update (paper §4.3) =====
             if e_norm > 0.01:
                 a_slope = max(0.01, (e_norm - 0.01) / e_norm)
             else:
                 a_slope = 0.01
             if a_slope <= 0.01 and pool_mode == 'avg':
                 pool_mode = 'max'
                 print(f'[Epoch {j} Batch {i}] Pooling switched avg→max (a={a_slope:.4f})')
