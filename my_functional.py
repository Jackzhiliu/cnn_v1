...
#     slope=torch.full(y.size(),s1).cuda()
...
def my_leaky_relu_dot(x, slope_val=None):
    """
    Derivative of LeakyReLU:
      if x > 0: 1
      else:      slope_val (or global s1 if slope_val is None)
    """
    # choose which slope to use
    if slope_val is None:
        slope_val = s1

    y = x.clone()
    one          = torch.ones_like(y)
    slope_tensor = torch.full_like(y, slope_val)  # same device & dtype as y
    return torch.where(y > 0, one, slope_tensor)
